...
router.get('/getComponents', (req, res, next) => {
    db.query(
        `SELECT * FROM components INNER JOIN users ON components.author_id = users.id;`,
        (err, result) => {
            if (err) {
                return res.status(400).send({
                    msg: err
...
router.post('/getComponent', (req, res, next) => {
    db.query(
        `SELECT * FROM components INNER JOIN users ON components.author_id = users.id WHERE component_id = ${db.escape(req.body.component_id)};`,
        (err, result) => {
            if (err) {
                return res.status(400).send({
                    msg: err
...
router.post('/saveComponent', userMiddleware.isLoggedIn, userMiddleware.validateExpertStatus, (req, res, next) => {
    if(req.body.component_id){
        db.query(
            `UPDATE components SET title = ${db.escape(req.body.title)}, code = ${db.escape(req.body.code)}, language = ${db.escape(req.body.language)}, last_update = ${db.escape(req.body.last_update)} WHERE component_id = ${db.escape(req.body.component_id)};`,
            (err, result) => {
                if (err) {
                    return res.status(400).send({
                        msg: err
        ...
        db.query(
            `INSERT INTO components (title, author_id, code, language, last_update) VALUES (${db.escape(req.body.title)}, ${db.escape(req.body.author_id)}, ${db.escape(req.body.code)}, ${db.escape(req.body.language)}, ${db.escape(req.body.last_update)});`,
            (err, result) => {
                if (err) {
                    return res.status(400).send({
                        msg: err
